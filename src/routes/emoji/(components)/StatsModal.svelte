<script lang="ts">
    ...
    function initChart() {
        ...
        chart = new chartjs(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: locale.t('pages.emoji.modals.stats.graph.label'),
    ...
    function handleModalOpen() {
        tick().then(() => { if (chartCanvas) initChart(); });
    }
...
<StylizedModal onOpenChange={(e) => e ? handleModalOpen() : handleModalClose()}>
    {#snippet trigger(toggleModal)}
        {@render button(toggleModal)}
    {/snippet}

    {#snippet title()}
        {locale.t('pages.emoji.modals.stats.title')}
    {/snippet}
        ...
        <div class="h-full sm:mx-[25%] mt-4 space-y-4 text-white">
            <div class="space-y-4 flex flex-col items-center md:flex-row md:space-y-0 md:justify-center md:space-x-4">
                <div class="text-center">
                    <p class="text-2xl">{locale.t('pages.emoji.modals.stats.metrics.games_won.label')}</p>
                    <p class="text-5xl">{stats.wins}</p>
                </div>
                <div class="text-center">
                    <p class="text-2xl">{locale.t('pages.emoji.modals.stats.metrics.average_tries.label')}</p>
                    <p class="text-5xl">{stats.average_tries.toFixed(3)}</p>
                </div>
                <div class="text-center">
                    <p class="text-2xl">{locale.t('pages.emoji.modals.stats.metrics.one_shots.label')}</p>
                    <p class="text-5xl">{stats.one_shots}</p>
                </div>
                <div class="text-center">
                    <p class="text-2xl">{locale.t('pages.emoji.modals.stats.metrics.current_streak.label')}</p>
                    <p class="text-5xl">{stats.current_streak}</p>
                </div>
                <div class="text-center">
                    <p class="text-2xl">{locale.t('pages.emoji.modals.stats.metrics.best_streak.label')}</p>
                    <p class="text-5xl">{stats.best_streak}</p>
                </div>
            </div>
            <div class="relative w-full h-[300px]">
                <canvas bind:this={chartCanvas} id="statsChart"
                    style="position: absolute; left: 0; top: 0; width: 100%; height: 100%;">
                    {locale.t('pages.emoji.modals.stats.graph.fallback')}
                </canvas>
            </div>
            <p class="text-xl text-center">{locale.t('pages.emoji.modals.stats.graph.title')}</p>
        </div>
    {/snippet}
</StylizedModal>
